...
const instructions = {
  polish: "Исправь стиль, грамматику и пунктуацию. Сделай текст ясным и естественным, сохрани смысл и факты.",
...
export const assistantRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post("/rewrite", async (request, reply) => {
    ...
    if (!parsed.success) return reply.code(400).send({ message: "Выделите текст от 2 до 12 000 символов" });
    try {
      const response = await fetch(`${env.OLLAMA_URL.replace(/\/$/, "")}/api/generate`, {
        ...
        body: JSON.stringify({
          ...
          stream: false,
          prompt: `Ты профессиональный русскоязычный редактор. ${instructions[parsed.data.action]} Верни только готовый результат без пояснений и внешних кавычек.\n\nТекст:\n${parsed.data.text}`,
      ...
      return reply.code(503).send({ message: `Локальная модель ${env.OLLAMA_MODEL} недоступна. Запустите Ollama.` });
    }
  });
};
