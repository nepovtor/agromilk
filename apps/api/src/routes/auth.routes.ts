...
import { loginSchema } from "@landing/shared";
...
export const authRoutes: FastifyPluginAsync = async (app) => {
  app.post("/login", { config: { rateLimit: { max: 10, timeWindow: "15 minutes" } } }, async (request, reply) => {
    ...
    const [admin] = await db.select().from(admins).where(and(eq(admins.email, email), eq(admins.isActive, 1))).limit(1);
  ...
  app.get("/google", { config: { rateLimit: { max: 20, timeWindow: "15 minutes" } } }, async (_request, reply) => {
    if (!googleEnabled) return reply.code(404).send({ error: "GOOGLE_AUTH_DISABLED", message: "Вход через Google не настроен" });
  ...
  app.get<{ Querystring: { code?: string; state?: string; error?: string } }>("/google/callback", async (request, reply) => {
    const clearState = () => reply.clearCookie(GOOGLE_STATE_COOKIE, { path: "/api/v1/auth/google/callback" });
    if (!googleEnabled) return redirectToLogin(clearState(), "not_configured");
    if (request.query.error) return redirectToLogin(clearState(), "cancelled");
    const signedState = request.cookies[GOOGLE_STATE_COOKIE];
    const savedState = signedState ? request.unsignCookie(signedState) : null;
    if (!request.query.code || !request.query.state || !savedState?.valid || savedState.value !== request.query.state) {
    ...
    try {
      ...
      if (!profile.email || profile.email_verified !== true) return redirectToLogin(reply, "email_not_verified");
  ...
  app.post("/logout", async (request, reply) => {
    ...
    if (signed) {
      ...
      if (unsigned.valid && unsigned.value) {
        await db.delete(adminSessions).where(eq(adminSessions.tokenHash, hashToken(unsigned.value)));
      }
    }
