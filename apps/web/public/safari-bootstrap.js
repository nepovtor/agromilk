(function () {
  ...
  function describe(value) {
    ...
    try { return String(value); } catch { return "Неизвестная ошибка"; }
  }
  ...
  function showFailure(message) {
    ...
    panel.style.cssText = "box-sizing:border-box;max-width:680px;margin:15vh auto;padding:28px;font:16px/1.5 -apple-system,BlinkMacSystemFont,sans-serif;color:#173247";
  ...
  window.setTimeout(function () {
    if (!rootIsEmpty()) return;
    showFailure(
      "Код приложения не выполнился за " + Math.round((Date.now() - startedAt) / 1000) +
      " с. JavaScript: " + (typeof Promise === "function" ? "доступен" : "отключён") + "."
    );
  }, 4000);
}());
